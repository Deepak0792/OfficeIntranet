...
-- =============================================================================================================
-- MODULES:
--   1. Master Data         : Designation, TimeZoneMaster, Country, LegalEntity, Region, OfficeLocation,
--                            Department, RelationshipType, DocumentType
...
CREATE TABLE ShiftSwapRequest (
    ...
    CONSTRAINT FK_ShiftSwapRequest_TargetEmployee
        ...
        REFERENCES Employee(Id)
    -- FK to EmployeeShiftRoster added after that table is created (see Module 8)
);
...
-- Add deferred FK from ShiftSwapRequest to EmployeeShiftRoster
...
CREATE TABLE AttendanceRegularization (
    ...
    CONSTRAINT FK_Regularization_Status
        ...
        REFERENCES AttendanceRegularizationStatus(Id)
);
...
-- RemainingDays is a computed column; linked to source attendance record
-- -------------------------------------------------------
...
ALTER TABLE MobileAttendanceLog
...
CREATE INDEX IX_Employee_DisplayName     ON Employee (DisplayName);

-- Employee Relationships
CREATE INDEX IX_EmployeeRelationship_Parent  ON EmployeeRelationship (ParentEmployeeId);
CREATE INDEX IX_EmployeeRelationship_Child   ON EmployeeRelationship (ChildEmployeeId);

-- Employee Mappings
CREATE INDEX IX_EmployeeSkill_Skill          ON EmployeeSkill (SkillId);
CREATE INDEX IX_EmployeeDepartment_Employee  ON EmployeeDepartment (EmployeeId);
CREATE INDEX IX_EmployeeTeam_Employee        ON EmployeeTeam (EmployeeId);
CREATE INDEX IX_EmployeeLocation_Employee    ON EmployeeLocation (EmployeeId);

-- Office Location
CREATE INDEX IX_OfficeLocation_Country   ON OfficeLocation (CountryId);
CREATE INDEX IX_OfficeLocation_Region    ON OfficeLocation (RegionId);

-- Documents
CREATE INDEX IX_EmployeeDocument_Employee       ON EmployeeDocument (EmployeeId);
CREATE INDEX IX_EmployeeDocument_DocumentType   ON EmployeeDocument (DocumentTypeId);
CREATE INDEX IX_EmployeeDocument_ExpiryDate     ON EmployeeDocument (ExpiryDate);

-- Time Zone
CREATE INDEX IX_TimeZoneMaster_TimeZoneCode         ON TimeZoneMaster (TimeZoneCode);
CREATE INDEX IX_TimeZoneMaster_IanaTimeZoneId        ON TimeZoneMaster (IanaTimeZoneId);
CREATE INDEX IX_TimeZoneMaster_WindowsTimeZoneId     ON TimeZoneMaster (WindowsTimeZoneId);

-- Scope-based Assignments
CREATE INDEX IX_ShiftAssignment_Scope        ON ShiftAssignment (ScopeTypeId, ScopeReferenceId);
CREATE INDEX IX_WorkWeekAssignment_Scope     ON WorkWeekPolicyAssignment (ScopeTypeId, ScopeReferenceId);
CREATE INDEX IX_RotationAssignment_Scope     ON RotationShiftAssignment (ScopeTypeId, ScopeReferenceId);
CREATE INDEX IX_HolidayAssignment_Scope      ON HolidayCalendarAssignment (ScopeTypeId, ScopeReferenceId);

-- Roster & Attendance
CREATE INDEX IX_Roster_Employee_Date         ON EmployeeShiftRoster (EmployeeId, RosterDate);
CREATE INDEX IX_Attendance_Employee_Date     ON AttendanceRecord (EmployeeId, AttendanceDate);
CREATE INDEX IX_AttendanceLog_Employee_Time  ON AttendanceLog (EmployeeId, PunchTime);

-- Holiday
CREATE INDEX IX_Holiday_Date                 ON Holiday (HolidayDate);

-- Leave
CREATE INDEX IX_LeaveRequest_Employee        ON LeaveRequest (EmployeeId, FromDate, ToDate);
CREATE INDEX IX_LeaveBalance_Employee        ON LeaveBalance (EmployeeId, LeaveTypeId);
...
-- Mobile Attendance
CREATE INDEX IX_MobileAttendanceLog_Employee ON MobileAttendanceLog (EmployeeId, PunchTime);

-- Shift Swap
CREATE INDEX IX_ShiftSwapRequest_Requester   ON ShiftSwapRequest (RequesterEmployeeId, RequestedAt);
